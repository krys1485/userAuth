...
// GET /reset-password  => show the reset form with token
exports.getResetPassword = (req, res) => {
  const { token } = req.query; // token comes from URL query params
  res.render('pages/reset-password', { token, message: null });
};

// POST /reset-password  => process the reset password form submission
exports.postResetPassword = async (req, res) => {
  const { token, newPassword } = req.body;  // from submitted form body
  ...
  try {
    ...
    if (!user || user.reset_token !== token) {
      return res.render('pages/reset-password', { message: 'Invalid or expired reset link', token });
    }
    ...
    res.render('pages/reset-password', { message: 'Password reset successfully! You can now log in.', token: null });
  } catch (error) {
    console.log('Error during password reset:', error);
    res.render('pages/reset-password', { message: 'An error occurred during password reset. Please try again.', token });
  }
}
